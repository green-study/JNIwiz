...
void Instrumentor::Instrument(const char* file_path) {
  ...
  // Set the target as C++ source file
  ...
  */
  ...
  if(rewrite_buf != nullptr) {
    ...
    std::cout << "There is nothing to rewrite." << std::endl;
  }
}
...
std::string Instrumentor::GetOutputName(llvm::StringRef fname) {
  ...
  if (fname.endswith(".c"))
    ...
    return std::string(fname.drop_back(2/*.h*/)) + "_instrumented.h";
  else 
    throw InvalidFileException(std::string(fname) + 
        ": file extension must be one of .c, .cc, .cpp or .h");
}
